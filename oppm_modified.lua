...
local raw_url = "ghfast.top/raw.githubusercontent.com"
...
local function getContent(url, detail)
  local sContent = ""
  if verbose then
    io.write((detail or "GET") .. " " .. url .. "\n")
  end
...
local getRepos = cached(function()
  local success, sRepos = pcall(getContent,"https://" .. raw_url .. "/OpenPrograms/openprograms.github.io/master/repos.cfg", "REPO")
  if not success then
...
local getPackages = cached(function(repo)
  local success, sPackages = pcall(getContent,"https://" .. raw_url .. "/" .. repo .. "/master/programs.cfg", "PKG ")
...
local function parseFolders(pack, repo, info)
  ...
  local function getFolderTable(repo, namePath, branch)
    local success, filestring = pcall(getContent,"https://" .. api_url .. "/repos/" .. repo .. "/contents/" .. namePath .. "?ref=" .. branch, "DIR ")
  ...
  local function unserializeFiles(files, repo, namePath, branch, relPath)
    ...
    for _,v in pairs(files) do
      if v["type"] == "file" then
        ...
        newPath = newPath:gsub("raw.githubusercontent.com", raw_url)
...
local function installPackage(pack,path,update,tPacks)
  ...
  for i,j in pairs(info.files) do
    ...
    local success,response = pcall(downloadFile,"https://" .. raw_url .. "/" .. repo .. "/" .. string.gsub(i,"^%?",""),nPath, nil, soft)
